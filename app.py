...
def fetch_and_read_file(file_path__, base_url):
    ...
    يقرأ محتوى الملف ويعيده مع طباعة تحققات داخلية.

    Args:
        file_path__ (str): المسار النسبي للملف (مثل "storage/books/filename.txt").
    ...
    storage_dir = os.path.join(os.getcwd(), 'public', 'storage', 'books')
    os.makedirs(storage_dir, exist_ok=True)  # للتأكد من وجود المجلد أو إنشائه إذا لم يكن موجودًا
    print(f"[INFO] مجلد التخزين: {storage_dir}")

    # تحديد المسار المحلي الكامل للملف (احفظ فقط اسم الملف لتجنب إنشاء مجلدات فرعية خاطئة)
    filename = os.path.basename(file_path__)
    local_file_path = os.path.join(storage_dir, filename)
    print(f"[INFO] المسار المحلي للملف: {local_file_path}")

    try:
        # التحقق من وجود الملف محليًا
        if not os.path.exists(local_file_path):
            print(f"[INFO] الملف غير موجود محليًا. محاولة تحميل الملف من: {base_url}/{file_path__}")
            file_url = f"{base_url}/{file_path__}"
            response = requests.get(file_url, stream=True)
            response.raise_for_status()  # تأكد من نجاح الطلب

            # حفظ الملف محليًا
            with open(local_file_path, 'wb') as file:
                for chunk in response.iter_content(chunk_size=8192):
                    file.write(chunk)
            print("[INFO] تم تحميل الملف وحفظه محليًا.")

        else:
            print("[INFO] الملف موجود محليًا، سيتم القراءة منه.")

        # قراءة محتوى الملف
        with open(local_file_path, 'r', encoding='utf-8') as file:
            content = file.read()
        print(f"[INFO] تم قراءة الملف. عدد الأحرف: {len(content)}")

        # تحقق إضافي لو النص فارغ
        if not content.strip():
            print("[WARNING] محتوى الملف فارغ!")

        return content

    except requests.exceptions.RequestException as e:
        error_msg = f"فشل تحميل الملف من الرابط: {e}"
        print(f"[ERROR] {error_msg}")
        return {"error": error_msg}

    except Exception as e:
        error_msg = f"حدث خطأ أثناء قراءة الملف: {e}"
        print(f"[ERROR] {error_msg}")
        return {"error": error_msg}
...
def clean_text(text):
    # ✅ التعامل مع أنواع غير نصية
    if isinstance(text, dict):
        # حاول استخراج النص إذا كان داخل مفتاح مثل "content" أو "text"
        text = text.get("content") or text.get("text") or ""

    if not isinstance(text, str):
        raise ValueError("❌ دالة clean_text تتوقع نصًا من نوع str، لكن تم تمرير نوع:", type(text))
    ...
    # استبدال الفاصلة العشرية بفاصلة عربية
    text = re.sub(r'(?<=\d),(?=\d)', '،', text)

    # إزالة الرموز غير المرغوبة
    text = re.sub(r'[^\w\s\.,،\؟\!]', '', text)

    # إزالة الإيموجي والزخارف
...
# ✅ تحميل نموذج التضمين (متعدد اللغات ويدعم العربية)
# 5s تشبيه 0.10
...
def retrieve_relevant_text(question, book_content, word_limit, min_words=50, top_k=10, similarity_threshold=0.9):
    ...
    if total_words_in_book <= word_limit:
        print("✅ النص أقصر من الحد المطلوب، سيتم إرجاعه كما هو.")
    ...
    def filter_similar_chunks(chunks_with_scores):
        texts = [chunk for chunk, _ in chunks_with_scores]
        ...
        for i in range(len(texts)):
            if i in used:
                continue
            filtered.append(chunks_with_scores[i])
            for j in range(i + 1, len(texts)):
                if cosine_similarity([embeddings[i]], [embeddings[j]])[0][0] > similarity_threshold:
                    used.add(j)
        return filtered

    def merge_chunks(chunks_with_scores, max_chunks=4):
        merged = ""
        total_words = 0
        count = 0
    ...
    chunks = split_text_into_chunks(book_content)
    if not chunks:
        print("❌ لا توجد فقرات صالحة.")
        return ""

    index, _, chunk_list = build_faiss_index(chunks)
    relevant_chunks = retrieve_relevant_chunks(question, index, chunk_list)
    if not relevant_chunks:
        print("❌ لم يتم العثور على فقرات مشابهة.")
        return ""
